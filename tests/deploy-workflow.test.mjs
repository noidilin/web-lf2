...
test('terraform plan workflow uses explicit matrix and sticky artifact-backed comments', () => {
  ...
  assert.match(terraformPlanWorkflow, /\| tee plan\.txt/);
  assert.match(terraformPlanWorkflow, /path: \$\{\{ matrix\.infra_dir \}\}\/plan\.txt/);
  assert.match(terraformPlanWorkflow, /PLAN_FILE: \$\{\{ matrix\.infra_dir \}\}\/plan\.txt/);
